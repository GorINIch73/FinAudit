...
*   **Управление Платежами:** Функционал добавления, обновления и удаления записей платежей, работа с расшифровками, контрагентами, договорами и КОСГУ.
*   **Управление Контрагентами:** Операции CRUD для записей контрагентов. Надежная логика импорта обрабатывает поиск только по имени и значения ИНН NULL. Для отфильтрованной выборки доступны групповые установка и снятие признака «Договор необязателен» с подтверждением, резервной копией и выполнением в транзакции.
*   **Управление Договорами:** Операции CRUD для записей договоров. Номера договоров при записи и поиске нормализуются без внешних и внутренних пробелов, что уменьшает появление дублей из-за различий в форматировании.
*   **Документы основания:** Импорт и сопоставление документов основания для сверки с банковскими платежами. Механика ЖО4 и документов основания пока дорабатывается и может требовать дополнительной проверки результатов.
*   **Импорт из TSV/CSV:** Расширенная функциональность импорта банковских файлов TSV и CSV. Анализирует детали платежей, автоматически создает/обновляет контрагентов, извлекает/связывает договоры из описаний платежей. Поля «Тип» и «Примечание» не сопоставляются автоматически и при необходимости выбираются вручную. **Включает возможность прерывания длительных операций импорта.**
*   **Импорт ЖО4 из TSV/CSV:** Загрузка данных ЖО4 для формирования документов основания и последующей сверки с банком. Диалог импорта одновременно показывает файлы с расширениями `.tsv` и `.csv`. Функциональность находится в рабочем, но еще не окончательно стабилизированном состоянии.
*   **Групповые операции (Договоры):**
    *   **Подтверждение операций:** Использует кастомный виджет подтверждения для групповых операций.
    *   **Фильтр "с реестровым номером":** Добавлен новый фильтр для отображения договоров с проставленным реестровым номером.
...
*   **Разбор назначения платежа:** Контрактные регулярные выражения выполняются с поддержкой Unicode. Распознаются сокращения и слитные варианты вида `К.номеротдата`, кириллические части номера и буквенные суффиксы; добавлены границы обозначений договора для защиты от совпадений внутри обычных слов.
*   **Регистронезависимый поиск:** Фильтры основных форм и поисковые выпадающие списки одинаково обрабатывают верхний и нижний регистр русского и латинского текста, включая `Ё/ё`.
...
*   **Экспорт запросов в ODS:** Кнопка «Открыть ODS» в универсальной форме результатов и форме SQL-запроса формирует электронную таблицу со всеми строками в текущем порядке и открывает её в приложении по умолчанию. Файл создаётся во временном каталоге; для постоянного хранения используйте «Сохранить как» в табличном редакторе.
*   **Печатные HTML-отчеты:** Формирование печатных HTML-представлений для сводных отчетов, которые можно открыть в браузере и распечатать или сохранить средствами браузера.
*   **Итоги по договорам для акта:** Доступны общий отчет и отчет в разрезе годов платежей с группировкой по году, КПС и КОСГУ. В отчеты включаются договоры с признаком «Для проверки»; показываются количество и стоимость договоров и фактически оплаченные суммы соответствующего года. Форма результатов при открытии и обновлении использует порядок `ORDER BY` запроса и не восстанавливает ранее сохраненную сортировку таблицы.
...
│   │   ├── BasePaymentsView.*  # Форма документов основания
│   │   ├── JO4ImportMapView.*  # Настройка импорта ЖО4
│   │   ├── ReconciliationView.* # Сверка банка и документов основания
│   │   ├── ExpenseStructureView.* # Анализ структуры расходов
...
*   Форма "Договоры" (включая групповые операции и фильтр по реестровому номеру) - **Завершено**
*   Документы основания и сверка с банком - **В работе**
*   Импорт ЖО4 из TSV - **В работе, механика требует стабилизации**
*   Импорт TSV банка (анализ договоров из описания) - **Завершено**
*   Прерывание длительных операций (групповые операции, импорт) - **Завершено**
*   Отчеты по расшифровкам (КОСГУ, Контрагенты, Банк) и печатные HTML-сводки - **Завершено**

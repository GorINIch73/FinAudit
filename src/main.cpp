        ...
        if (ImGui::BeginMainMenuBar()) {
            ...
            if (ImGui::BeginMenu(ICON_FA_FILE_PDF " Отчеты")) {
                ...
                if (ImGui::MenuItem(ICON_FA_TRIANGLE_EXCLAMATION
                                    " Риск-анализ")) {
                    uiManager.CreateSpecialQueryView("Риск-анализ",
...
SELECT
    82 AS risk_score,
    'Платеж по ненайденному договору на усиленном контроле' AS risk_rule,
    'payment' AS object_type,
    p.id AS object_id,
    p.date,
    IFNULL(p.doc_number, '') AS number,
...
LEFT JOIN Counterparties pc ON pc.id = p.counterparty_id
WHERE p.type = 0
  AND IFNULL(ctr.is_for_special_control, 0) = 1
  AND IFNULL(ctr.is_found, 0) = 0

UNION ALL
SELECT
    72 AS risk_score,
    'Платеж по ненайденному договору с примечанием' AS risk_rule,
    'payment' AS object_type,
    p.id AS object_id,
    p.date,
    IFNULL(p.doc_number, '') AS number,
    ...
    IFNULL(pc.name, '') AS counterparty,
    'Договор: ' || ctr.number || ' от ' || ctr.date ||
        '; примечание: ' || substr(IFNULL(ctr.note, ''), 1, 220) AS details
FROM PaymentDetails pd
JOIN Payments p ON p.id = pd.payment_id
JOIN Contracts ctr ON ctr.id = pd.contract_id
LEFT JOIN Counterparties pc ON pc.id = p.counterparty_id
WHERE p.type = 0
  AND trim(IFNULL(ctr.note, '')) <> ''
  AND IFNULL(ctr.is_found, 0) = 0
